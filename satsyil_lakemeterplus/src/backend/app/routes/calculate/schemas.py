...
class LakebaseCalculationRequest(BaseModel):
    ...
    tier: str = Field(...)
    cu_size: float = Field(...)
    compute_mode: str = Field(default="autoscale", description="autoscale or fixed")
    min_cu: Optional[float] = Field(None, ge=0, description="Minimum autoscaling CU")
    max_cu: Optional[float] = Field(None, ge=0, description="Maximum autoscaling CU")
    scale_to_zero_enabled: bool = Field(default=False, description="Whether Lakebase can scale to zero")
    active_hours_per_month: Optional[float] = Field(None, ge=0, description="Active hours when scale-to-zero is enabled")
    scale_up_hours_per_month: Optional[float] = Field(None, ge=0, description="Hours spent above minimum CU")
    always_on_discount_pct: float = Field(default=25.0, ge=0, le=100, description="Discount on always-on minimum CU")
    read_replicas: int = Field(default=0, ge=0)
    num_nodes: Optional[int] = Field(None, ge=1, le=4, description="Total nodes including primary and up to 3 read replicas")
    hours_per_day: Optional[float] = Field(None, ge=0)
    days_per_month: Optional[int] = Field(None, ge=1, le=31)
    hours_per_month: Optional[float] = Field(None, ge=0)
