#!/bin/bash
# Lakemeter Installer (merged: estimation + cost-observability module)
#
# Ported from lakemeter-oss's scripts/install.sh (see
# ../../docs/merge-tasks.md task #13). This is the installer entrypoint
# actually wired to scripts/databricks.yml's `lakemeter_installer` job and
# scripts/notebooks/ — chosen over lakemeter-oss's other installer
# (install_lakemeter.py, a standalone Python reimplementation of the same
# flow) because this one's task-key list and --params already matched the
# DAB/notebooks that were merged first; install_lakemeter.py was NOT
# ported. That's a judgment call, not a settled decision from either
# source repo (both installers existed there simultaneously with no
# documented authority between them) — see docs/TODO.md.
#
# Extends the original with four new prompts/flags for the ported
# cost-observability module (--warehouse-id, --catalog-name, --mock-mode,
# --admin-users), threaded into the same `databricks bundle run
# lakemeter_installer --params ...` call already used for the estimation
# module's config — see scripts/databricks.yml and
# scripts/notebooks/05a_create_app.py.
#
...
#   ./scripts/install.sh --non-interactive             # Use all defaults (mock_mode=true demo)
#   ./scripts/install.sh --profile my-profile          # Specify CLI profile
#   ./scripts/install.sh --non-interactive --profile p --warehouse-id abc123 --mock-mode false
...
# Observability module (see docs/merge-tasks.md task #13)
WAREHOUSE_ID=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  --warehouse-id ID      Observability module: SQL Warehouse ID (default: none)"
            echo "  --catalog-name NAME    Observability module: Unity Catalog name (default: workspace)"
            echo "  --mock-mode true|false Observability module: use mock system tables (default: true)"
            echo "  --admin-users EMAILS   Observability module: comma-separated admin panel emails (default: none)"
...
if [ "$NON_INTERACTIVE" = false ]; then
    ...
    echo ""
    echo -e "  ${BOLD}Cost-observability module${NC} (Unity Catalog system-table dashboards)"

    read -p "  Use mock/demo data? (true/false) [$MOCK_MODE]: " input
    MOCK_MODE="${input:-$MOCK_MODE}"

    if [ "$MOCK_MODE" != "true" ]; then
        read -p "  SQL Warehouse ID (required when mock-mode=false): " input
        WAREHOUSE_ID="${input:-$WAREHOUSE_ID}"
    else
        read -p "  SQL Warehouse ID [optional while mock-mode=true]: " input
        WAREHOUSE_ID="${input:-$WAREHOUSE_ID}"
    fi

    read -p "  Unity Catalog name [$CATALOG_NAME]: " input
    CATALOG_NAME="${input:-$CATALOG_NAME}"

    read -p "  Admin panel emails, comma-separated [$ADMIN_USERS]: " input
    ADMIN_USERS="${input:-$ADMIN_USERS}"
fi

if [ "$MOCK_MODE" != "true" ] && [ -z "$WAREHOUSE_ID" ]; then
    echo ""
    echo -e "${YELLOW}Warning: mock-mode is false but no warehouse-id was given — the${NC}"
    echo -e "${YELLOW}observability module will not be able to run billing queries until${NC}"
    echo -e "${YELLOW}DATABRICKS_WAREHOUSE_ID is set in the deployed app.${NC}"
fi
...
echo -e "  Warehouse ID:     ${CYAN}${WAREHOUSE_ID:-(none)}${NC}"
echo -e "  Catalog name:     ${CYAN}${CATALOG_NAME}${NC}"
echo -e "  Admin users:      ${CYAN}${ADMIN_USERS:-(none)}${NC}"
echo ""
...
# Includes the observability module (backend/app/observability/) since it
# lives under backend/app/ like the rest of the merged app.
...
echo -e "  configure the app (including the cost-observability module), and"
echo -e "  deploy it."
echo ""
...
databricks bundle run lakemeter_installer $PROFILE_FLAG \
    --params "instance_name=$INSTANCE_NAME,db_name=$DB_NAME,app_name=$APP_NAME,secrets_scope=$SECRETS_SCOPE,claude_endpoint=$CLAUDE_ENDPOINT,warehouse_id=$WAREHOUSE_ID,catalog_name=$CATALOG_NAME,mock_mode=$MOCK_MODE,admin_users=$ADMIN_USERS" \
